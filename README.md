...
1. 상품 목록에서 수량 조절
   - CartScreen의 각 상품에서 장바구니 담기/수량 조절이 가능해야 한다.
   - 담기 전: `+` FAB가 노출된다.
   - `+`를 누르면 Cart에 상품이 추가되고, CountComponent(± 버튼)가 노출된다.
   - 수량이 0이 되면(1에서 `-`) 다시 `+` FAB가 노출된다.
2. 화면 간 수량 동기화
   - 상품 목록에서 수량을 변경하면 장바구니 화면에도 동일하게 반영된다.
   - 장바구니 화면에서 수량을 변경하면 상품 목록에도 동일하게 반영된다.
3. 반복되는 뷰(상품 수량 조절)를 재사용할 수 있는 방법을 고민해 본다.
   - CountComponent/ButtonComponent 분리
